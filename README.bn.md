...
আগে থেকে সেটআপ করা আছে? এক কমান্ডে রিপো pull হবে, dsh সঠিক `--allow-scripts`
তালিকা দিয়ে আপডেট হবে, আর কোন প্রজেক্টে হাত দিতে হবে সেটাও বলে দেবে:
...
প্রজেক্ট ফোল্ডারগুলো আর্গুমেন্টে দিন — অথবা একবার `projects.txt`-এ লিখে রাখুন
(git-ignored), তাহলে শুধু `update.sh` চালালেই প্রতিবার দেখে নেবে:
...
**কেন শুধু জানায়, নিজে বদলায় না:** প্রতি প্রজেক্টে `AGENTS.md`-এর আলাদা কপি
থাকে, আর তাতে `## Project specifics` সেকশনে আপনার নিজের লেখা থাকে — এই রিপো
যার কিছুই জানে না। Overwrite করলে ওটা মুছে যেত। তাই স্ক্রিপ্ট version stamp
মিলিয়ে দেখে আর `diff` দেখিয়ে দেয়; বদলে যাওয়া অংশটুকু আপনি কপি করেন।

দুটো জিনিস কোনো স্ক্রিপ্ট করতে পারে না: **session রিস্টার্ট** (বদলানো নিয়ম শুধু
পরে খোলা session-এ পৌঁছায়), আর **`~/.dsh/settings.yaml` আপডেট** — মডেল, reasoning
effort আর API key প্রতি মেশিনের, রিপোর বাইরে।
...
| `update.bat` / `update.sh` | চালু সেটআপ আপডেট, পুরোনো `AGENTS.md` চিহ্নিত করা      |
